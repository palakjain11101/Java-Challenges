...
public class Tester {
	...
	/* since the methods in the door class were declared public, we can call them 
	in this class also; attachLock(), openDoor(), etc.are all called from the 
	Door class */ 	
  
        Door d= new Door();
        d.setRoomName("Research Labs");

        CardLock c= new CardLock();
        d.attachLock(c);

        SmartCard cardA = new SmartCard("student");/* to create a new student
 	SmartCard, student must be defined as a string */ 
        c.swipeCard(cardA);
        d.openDoor();

        SmartCard cardB = new SmartCard("staff");
	cardB.setStaff(true);//staff boolean status should be set as true before swiping the SmartCard
        c.swipeCard(cardB);                                                        
        d.openDoor(); 
